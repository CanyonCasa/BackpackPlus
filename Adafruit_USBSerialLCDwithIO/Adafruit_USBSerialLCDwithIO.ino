...
Embellished by CanyonCasa of Enchanted Engineering to provide 
General Purpose Inputs vs just Outputs. See README_WITH_IO.md
****************************************/
...
// The GPIO pins available on the Backpack 
#define GPIO_1   8 // PB0
#define GPIO_2  20 // PC2
#define GPIO_3  19 // PC4
#define GPIO_4  16 // PC7
uint8_t GPIO[] = {GPIO_1, GPIO_2, GPIO_3, GPIO_4};
...
#define MATRIX_SAVECUSTOMCHARBANK 0xC1  // 10 args: char #, bank, 8 bytes data
...
/***** GPIO commands */
#define MATRIX_GPIO_OFF 0x56
#define MATRIX_GPIO_ON 0x57
#define MATRIX_GPIO_START_ONOFF 0xC3

/***** Extra Adafruit commands */
#define MATRIX_GPI_READ 0x55 // 1 arg: mask; low nibble ANDs to select bits, high nibble XORs with low result
...
// 110 thru 430 !
#define GPIO_1_START_ONOFF_ADDR 431
#define GPIO_2_START_ONOFF_ADDR 432
#define GPIO_3_START_ONOFF_ADDR 433
#define GPIO_4_START_ONOFF_ADDR 434
#define GPI_MASK_ADDR 435
...
uint8_t gpiPort = 0;     // last GPI pin states
uint8_t gpiMask = 0xFF;  // read mask default
...
/*** test ****
  ...
  //for(uint8_t i=0; i<4; i++)        // set all GPIO to 'LOW' default
  // EEPROM.write(GPIO_1_START_ONOFF_ADDR+i, O);
/*** test ****/

  // configure all GPIO as defined in EEPROM, default "safe" INPUT_PULLUP
  for(uint8_t i=0; i<4; i++) {
    gpiPinMode(i,EEPROM.read(GPIO_1_START_ONOFF_ADDR+i));
  }
  gpiMask = EEPROM.read(GPI_MASK_ADDR);
  gpiPort = gpiRead(); // get intial state after setting up pins
      ...
      case MATRIX_SETCURSOR_POSITION:
       ...
       case MATRIX_CHANGESPLASH:
         for (uint8_t i=0; i < ROWS; i++) {
            for (uint8_t j=0; j<COLS; j++) {
              c = serialBlockingRead();
              EEPROM.write(SPLASH_ADDR + i * COLS + j, c); 
            }
          }
         break; 
       ...
       case MATRIX_SAVECUSTOMCHARBANK:
         ...
         readCustom(c, b);
         break;
        case MATRIX_GPI_READ:
          a = serialBlockingRead();
          if (gpiMask!=a) {
            gpiMask = a;
            EEPROM.write(GPI_MASK_ADDR, gpiMask);
          }
          gpiPort = gpiRead();  // update or a mask change may cause an output on check
          gpiSend(gpiPort);     // return current state
          break;
        ...
        case MATRIX_GPIO_ON:
          ...
          break;
        case MATRIX_GPIO_START_ONOFF:
          a = serialBlockingRead();
          b = serialBlockingRead();
          a--;
          if (a >= 4) return;
          EEPROM.write(GPIO_1_START_ONOFF_ADDR+a, b);
          break;
...
uint8_t getContrast() {
  return EEPROM.read(CONTRAST_ADDR);
}
...
void gpiPinMode(uint8_t bit, uint8_t mode) {
  switch (mode) {
    case 0x00: 
  ...
  if (mode!=EEPROM.read(GPIO_1_START_ONOFF_ADDR+bit)) // update EEPROM
    EEPROM.write(GPIO_1_START_ONOFF_ADDR+bit, mode);
}

// read all the GPIO pins as a port...
uint8_t gpiRead() {
  uint8_t p = 0;
  p = digitalRead(GPIO_4);
  p = p<<1 | digitalRead(GPIO_3);
  p = p<<1 | digitalRead(GPIO_2);
  p = p<<1 | digitalRead(GPIO_1);
  p = (p&gpiMask)^(gpiMask>>4);
  return p;
}

// report GPIO port state to USB and UART
void gpiSend(uint8_t p) {
  Serial.print(F("#0x0"));
